...
class LLMAssistantAggregator(LLMContextAggregator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # True once BotStartedSpeakingFrame has been observed since the last
        # reset. Used by push_aggregation() to suppress phantom assistant
        # turns — i.e. text that was generated by the LLM but whose TTS
        # output was killed (interruption, timeout, transport drop) before
        # any audio reached the user. Without this gate, the LLM context
        # and conversation logs would be polluted with turns the user
        # never heard.
        self._bot_started_speaking_since_reset = False
    ...
    async def reset(self):
        ...
        # Each new aggregation window starts with the bot not yet speaking;
        # gets flipped to True when BotStartedSpeakingFrame arrives.
        self._bot_started_speaking_since_reset = False
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, StartFrame):
            ...
            # Mark that real audio has begun playing for the current
            # aggregation window. push_aggregation() uses this to suppress
            # phantom assistant turns whose TTS output never reached the
            # user (e.g. interrupted before first audio frame left the
            # transport). BotStartedSpeakingFrame broadcasts both up and
            # down from base_output, so we'll see it here regardless of
            # direction.
            self._bot_started_speaking_since_reset = True
            await self.push_frame(frame, direction)
    ...
    async def push_aggregation(self) -> str:
        ...
        if aggregation and not self._bot_started_speaking_since_reset:
            ...
            logger.warning(
                f"{self} push_aggregation: dropping phantom assistant turn "
                f"(no BotStartedSpeakingFrame observed during this window) — "
                f"aggregation_chars={len(aggregation)}, "
                f"preview={aggregation[:80]!r}"
            )
            await self.reset()
            return ""
