...
class GoogleAssistantContextAggregator(OpenAIAssistantContextAggregator):
    async def handle_aggregation(self, aggregation: str):
        # Record the index where the textual assistant message will be added so that we can
        # potentially reorder any function-call messages that belong to the same LLM response
        # _after_ that text. This mirrors the logic in ``LLMAssistantContextAggregator`` where the
        # generic ``_reorder_context_for_response`` helper lives.

        text_msg_index = len(self._context.get_messages())

        # Add the textual response generated by Gemini.
    ...
    async def handle_function_call_in_progress(self, frame: FunctionCallInProgressFrame):
        """Add function call and placeholder response messages to the context.

        We also keep track of the indices of the messages we have just appended so that the
        generic ``_reorder_context_for_response`` implementation in
        ``LLMAssistantContextAggregator`` can later reposition them (i.e. move the function
        call/result messages *after* the textual assistant response).
        """

        # Capture current number of messages so that we can later compute the indices of the
        # messages we append below.
        messages_before = len(self._context.get_messages())

        # 1. Function call requested by the model.
        ...
        # 2. Immediate stub "tool" response indicating the function execution is in progress.
        self._context.add_message(
            Content(
                role="user",
                parts=[
...
class GoogleLLMService(LLMService):
    ...
    async def _process_context(self, context: OpenAILLMContext):
        ...
        try:
            ...
            # Flag to ensure we send LLMGeneratedTextFrame only once per response
