...
@pytest.mark.asyncio
async def test_mock_llm_function_calling_in_text():
    """Test that mock LLM service correctly processes function call chunks."""
    # Create function call chunks
    chunks = MockLLMService.create_text_chunks("Hello There. I can totally understand where are you coming from. <function_name>move_to_main_agenda</function_name>")
    ...
    async def move_to_main_agenda(params: FunctionCallParams):
        nonlocal function_called, received_params
        function_called = True
        received_params = params
        return {"result": "ok"}
    ...
    # Register function
    llm.register_function("move_to_main_agenda", move_to_main_agenda)

    # Create tools schema
    move_to_main_agenda_function = FunctionSchema(
        name="move_to_main_agenda",
        description="Move to main agenda",
        properties={},
        required=[]
    )
    tools = ToolsSchema(standard_tools=[move_to_main_agenda_function])

    # Create context with tools
    messages = [
        {"role": "system", "content": "You are a helpful weather assistant."},
        {"role": "user", "content": "Why are you wasting my time?"},
    ]
    context = LLMContext(messages, tools)
...
@pytest.mark.asyncio
async def test_mock_llm_mixed_text_and_function():
    """Test mixed text and function call chunks."""
    # Create mixed chunks
    chunks = MockLLMService.create_mixed_chunks(
    ...
    llm.register_function(None, mock_weather)  # Register as default handler
    ...
    full_text = "".join(f.text for f in text_frames)
    assert "I'll check the weather for you. " in full_text

    # Verify function call info frame
    info_frames = [f for f in received_frames if isinstance(f, FunctionCallsFromLLMInfoFrame)]
    assert len(info_frames) == 1
    assert len(info_frames[0].function_calls) == 1
    assert info_frames[0].function_calls[0].function_name == "get_current_weather"
...
@pytest.mark.asyncio
async def test_mock_llm_update_chunks():
    """Test updating mock chunks between calls."""
    # Start with text response
    initial_chunks = MockLLMService.create_text_chunks("First response")
    llm = MockLLMService(mock_chunks=initial_chunks, chunk_delay=0.001)

    # First call
    messages = [{"role": "user", "content": "First question"}]
    context1 = LLMContext(messages)

    pipeline = Pipeline([llm])
    frames_to_send = [LLMContextFrame(context1)]

    received_frames1, _ = await run_test(
        pipeline, frames_to_send=frames_to_send, expected_down_frames=None
    )

    text_frames1 = [f for f in received_frames1 if isinstance(f, LLMTextFrame)]
    text1 = "".join(f.text for f in text_frames1)
    assert "First response" in text1

    # Update chunks for second call
    new_chunks = MockLLMService.create_text_chunks("Second response")
    llm.set_mock_chunks(new_chunks)

    # Second call
    messages = [{"role": "user", "content": "Second question"}]
    context2 = LLMContext(messages)
    frames_to_send = [LLMContextFrame(context2)]

    received_frames2, _ = await run_test(
        pipeline, frames_to_send=frames_to_send, expected_down_frames=None
    )

    text_frames2 = [f for f in received_frames2 if isinstance(f, LLMTextFrame)]
    text2 = "".join(f.text for f in text_frames2)
    assert "Second response" in text2
    assert "First response" not in text2
